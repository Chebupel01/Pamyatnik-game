...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if button_x <= mouse_x <= button_x + button_width and button_y <= mouse_y <= button_y + button_height:
                # Если пользователь попал в кнопку, выходим из игры
                pygame.quit()
                sys.exit()
    ...
    # Отрисовываем текст поздравления
    congratulation_text = "Поздравляем! Вы прошли уровень!"
    congratulation_text_surface = font.render(congratulation_text, True, (0, 0, 0))
    screen.blit(congratulation_text_surface, (SCREEN_WIDTH // 2 - congratulation_text_surface.get_width() // 2, SCREEN_HEIGHT // 2 - congratulation_text_surface.get_height() // 2))
